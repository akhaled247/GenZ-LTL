...
import utils
from model.model_ma import build_model_safety_ma
from envs import make_env_safety_ma, get_env_attr
...
class Trainer:
    ...
    def train(self, log_csv: bool = True, log_wandb: bool = False):
        ...
        model = build_model_safety_ma(envs[0], training_status, model_configs[self.args.model_config])
    ...
    def make_envs(self, curriculum_stage: int) -> list[gymnasium.Env]:
        ...
        envs = [self.make_probe_env(curriculum_stage) for _ in range(self.args.experiment.num_procs)]
        ...
        for env, seed in zip(envs, seeds):
            env.reset(seed=seed)
        self.text_logger.info("MA safety environments loaded.")
        return envs
...
def parse_arguments() -> argparse.Namespace:
    ...
    args.experiment.multi_agent = True
    return args
