...
import utils
from model.model_ma import build_model_ma
from envs import make_env_ma, get_env_attr
...
class Trainer:
    ...
    def train(self, log_csv: bool = True, log_wandb: bool = False):
        ...
        model = build_model_ma(envs[0], training_status, model_configs[self.args.model_config])
        model.to(self.args.experiment.device)
        print(model.ltl_net)
        algo = torch_ac.MAIPPO(
    ...
    def make_envs(self, curriculum_stage: int) -> list[gymnasium.Env]:
        ...
        for env, seed in zip(envs, seeds):
            env.reset(seed=seed)
        self.text_logger.info("MA environments loaded.")
        return envs
